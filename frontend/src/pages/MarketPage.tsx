...
export function MarketPage() {
  ...
  if (loadingMarket) {
    return <div className="flex justify-center items-center min-h-screen bg-[#0D1117]"><Loader2 className="animate-spin text-cyan-400" size={32} /></div>;
  }
  ...
  return (
    <>
      ...
      <div className="bg-[#0D1117] min-h-screen text-white">
        {/* Top Header */}
        <div className="border-b border-gray-800/50 bg-[#161B22]">
          <div className="max-w-[1920px] mx-auto px-4 py-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                <button onClick={() => navigate('/markets')} className="text-gray-400 hover:text-white">
                  <ArrowLeft className="w-5 h-5" />
                </button>
                <div className="flex items-center gap-3">
                  {displayMarket.image_url && <img src={displayMarket.image_url} alt="" className="w-7 h-7 rounded-full" />}
                  <h1 className="text-base font-medium">{displayMarket.description}</h1>
                </div>
                <div className="flex items-center gap-6 text-sm">
                  <div>
                    <span className="text-gray-400">Chance</span>
                    <span className="ml-2 text-cyan-400 font-semibold">{yesPrice}%</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Liquidity</span>
                    <span className="ml-2 font-semibold">${parseFloat(displayMarket.volume || "0").toLocaleString()}</span>
                  </div>
                  <div>
        ...
        <div className="max-w-[1920px] mx-auto">
          <div className="grid grid-cols-[85%_15%] gap-0">
            ...
            <div className="border-r border-gray-800/50 h-full">
              <div className="grid grid-cols-[82.35%_17.65%]">
                {/* Chart Area */}
                <div>
                  <div className="border-b border-gray-800/50 px-4 py-3 bg-[#0D1117]">
                    <div className="text-2xl font-bold text-cyan-400">{yesPrice}% Chance</div>
                  </div>
                  ...
                  <div className="p-4 bg-[#0D1117]">
                    <div className="flex items-center gap-2 mb-4">
                      {(['5m', '15m', '1h', '5h', '1d', '1w', 'all'] as const).map((range) => (
                        <button
                          ...
                          className={`px-3 py-1 text-xs font-medium rounded transition-colors ${timeRange === range ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white hover:bg-gray-800'
                            }`}
                        >
                          {range.toUpperCase()}
                        </button>
                    ...
                    <div className="h-[500px] bg-[#0D1117]">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={getFilteredData().length > 0 ? getFilteredData() : [{ date: 'Start', YES: 50 }, { date: 'Now', YES: yesPrice }]}>
                          ...
                          <Line type="monotone" dataKey="YES" stroke="#06B6D4" strokeWidth={2} dot={false} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="border-l border-gray-800/50 bg-[#161B22]">
                  <div className="px-3 py-3 border-b border-gray-800/50">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => setSelectedOutcome(0)} className={`flex-1 px-2 py-1 text-xs font-medium rounded transition-colors ${selectedOutcome === 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-[#0D1117] text-gray-400 hover:text-white'}`}>Yes</button>
                      <button onClick={() => setSelectedOutcome(1)} className={`flex-1 px-2 py-1 text-xs font-medium rounded transition-colors ${selectedOutcome === 1 ? 'bg-red-500/20 text-red-400' : 'bg-[#0D1117] text-gray-400 hover:text-white'}`}>No</button>
                    </div>
                  </div>
                    ...
                    <div className="max-h-[500px] overflow-y-auto">
                      ...
                      <div className="py-2 text-center border-y border-gray-800/50 mb-2">
                        <div className="text-[10px] text-gray-400">
                          Spread: <span className="text-cyan-400 font-mono">{filteredAsks.length > 0 && filteredBids.length > 0 ? ((filteredAsks[0].price - filteredBids[0].price) * 100).toFixed(2) : '0.00'}¢</span>
                        </div>
                      </div>

                      <div className="space-y-0.5">
                        {filteredBids.slice(0, 10).map((bid, i) => (
                          <div key={i} className="grid grid-cols-[1fr_0.8fr_1fr] gap-2 text-xs py-1 hover:bg-emerald-500/10 rounded px-1 cursor-pointer">
                            <div className="text-emerald-400 font-mono font-semibold">{(bid.price * 100).toFixed(1)}¢</div>
              ...
              {/* Bottom Tabs */}
              <div className="border-t border-gray-800/50 bg-[#161B22]">
                <div className="flex border-b border-gray-800/50">
                  {['Positions', 'Open Orders', 'TWAP', 'Trade History', 'Funding History', 'Order History'].map((tab) => (
                    <button key={tab} onClick={() => setActiveTab(tab.toLowerCase().replace(' ', '-'))} className={`px-4 py-2.5 text-xs font-medium transition-colors ${activeTab === tab.toLowerCase().replace(' ', '-') ? 'text-white border-b-2 border-cyan-500' : 'text-gray-400 hover:text-white'}`}>
                      {tab}
                    </button>
                  ))}
                </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between p-3 bg-[#0D1117] rounded">
                            <div>
                              ...
                              <div className="text-sm font-semibold text-emerald-400">{parseFloat(positions[0]).toFixed(2)} shares</div>
                            </div>
            ...
            <div className="bg-[#161B22] flex flex-col h-full">
              <div className="flex border-b border-gray-800/50">
                <button onClick={() => setTradeType('buy')} className={`flex-1 py-3 text-sm font-semibold transition-colors ${tradeType === 'buy' ? 'text-white border-b-2 border-cyan-500' : 'text-gray-500 hover:text-white'}`}>Buy</button>
                <button onClick={() => setTradeType('sell')} className={`flex-1 py-3 text-sm font-semibold transition-colors ${tradeType === 'sell' ? 'text-white border-b-2 border-cyan-500' : 'text-gray-500 hover:text-white'}`}>Sell</button>
              </div>

              <div className="p-3 border-b border-gray-800/50">
                <div className="flex gap-2">
                  {(['market', 'limit', 'pro'] as const).map((type) => (
                    <button key={type} onClick={() => setOrderType(type)} className={`px-4 py-1.5 text-xs font-medium rounded transition-colors ${orderType === type ? 'bg-[#1A2332] text-cyan-400 border border-cyan-500/30' : 'text-gray-400 hover:text-white bg-[#0D1117] border border-gray-700'}`}>
              ...
              <div className="p-3">
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setSelectedOutcome(0)} className={`p-2.5 rounded-lg border-2 transition-all flex items-center justify-between ${selectedOutcome === 0 ? 'bg-emerald-500/10 border-emerald-500' : 'bg-[#0D1117] border-gray-700 hover:border-emerald-500/50'}`}>
                    <span className="text-[10px] font-medium text-gray-400">Yes</span>
                    <span className={`text-base font-bold ${selectedOutcome === 0 ? 'text-emerald-400' : 'text-gray-300'}`}>{yesPrice}.00¢</span>
                  </button>
                  <button onClick={() => setSelectedOutcome(1)} className={`p-2.5 rounded-lg border-2 transition-all flex items-center justify-between ${selectedOutcome === 1 ? 'bg-red-500/10 border-red-500' : 'bg-[#0D1117] border-gray-700 hover:border-red-500/50'}`}>
              ...
              <div className="px-3 pb-3">
                ...
                <div className="relative">
                  <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-[#0D1117] border border-gray-700 rounded px-3 py-2.5 text-sm text-white focus:border-cyan-500 focus:outline-none pr-16" placeholder="$0.00" />
              ...
              {orderType === 'limit' && (
                <div className="px-3 pb-3">
                  ...
                  <input
                    ...
                    className="w-full bg-[#0D1117] border border-gray-700 rounded px-3 py-2.5 text-sm text-white focus:border-cyan-500 focus:outline-none"
                    placeholder="0.50"
                    step="0.01"
                  />
                </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  {[{ val: 0, label: '0%' }, { val: 25, label: '25%' }, { val: 50, label: '50%' }, { val: 75, label: '75%' }, { val: 100, label: 'MAX' }].map((item) => (
                    <button
                      ...
                      className={`flex-1 px-2 py-1 text-[10px] font-medium rounded transition-colors ${sliderValue === item.val
              ...
              <div className="px-3 pb-3">
                <button className="w-full py-3 rounded-lg font-semibold text-sm transition-colors bg-cyan-500 hover:bg-cyan-600 text-black">Connect to Trade</button>
              </div>

              <div className="px-3 pb-4 border-b border-gray-800/50">
                <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={takeProfitStopLoss}
                    onChange={(e) => setTakeProfitStopLoss(e.target.checked)}
                    className="w-4 h-4 rounded border-gray-700 bg-[#0D1117]"
                  />
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <div className="relative">
                          <input
                            type="number"
                            className="w-full bg-[#0D1117] border border-gray-700 rounded px-2 py-1.5 text-xs text-white focus:border-cyan-500 focus:outline-none pr-6"
                            placeholder="0"
                          />
              ...
              <div className="p-4 flex-1 flex flex-col justify-end">
                ...
                {relatedMarketsExpanded && (
                  <>
                    <div className="space-y-2">
                      <div className="flex items-center gap-3 p-2 rounded hover:bg-[#0D1117] cursor-pointer transition-colors">
                        ...
                        <div className="text-sm font-bold text-cyan-400">1%</div>
                      </div>
                      <div className="flex items-center gap-3 p-2 rounded hover:bg-[#0D1117] cursor-pointer transition-colors">
                        ...
                        <div className="text-sm font-bold text-cyan-400">18%</div>
                      </div>
                      <div className="flex items-center gap-3 p-2 rounded hover:bg-[#0D1117] cursor-pointer transition-colors">
                        ...
                        <div className="text-sm font-bold text-cyan-400">2%</div>
                      </div>
                    </div>
                    <div className="flex gap-2 mt-4 overflow-x-auto">
                      {['All', 'Trump', 'Politics', 'Culture'].map((cat) => (
                        <button key={cat} className={`px-3 py-1 text-xs font-medium rounded transition-colors whitespace-nowrap ${cat === 'All' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-[#0D1117] text-gray-400 hover:text-white'}`}>{cat}</button>
                      ))}
                    </div>
                  </>
                )}
