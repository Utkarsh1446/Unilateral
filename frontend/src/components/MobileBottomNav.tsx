import { TrendingUp, Users, Wallet, User, Compass, Activity } from 'lucide-react';
...
export function MobileBottomNav() {
  ...
  const navItems = [
    ...
    { icon: Activity, label: 'Whales', path: '/whales' },
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center justify-center gap-1 px-4 py-2 transition-colors flex-1 ${isActive ? 'text-foreground' : 'text-muted-foreground'
                }`}
            >
