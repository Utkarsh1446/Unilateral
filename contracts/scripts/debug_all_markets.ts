...
async function main() {
    ...
    // New market from latest attempt - need to find it
    ...
    for (let i = Math.max(0, events.length - 3); i < events.length; i++) {
        ...
        try {
            ...
            console.log("OrderBook YES:", ethers.formatUnits(obBal0, 6));
            console.log("OrderBook NO:", ethers.formatUnits(obBal1, 6));
        } catch (e: any) {
    ...
    // Show last 3 orders
    const startIdx = Math.max(0, Number(nextOrderId) - 3);
    for (let i = startIdx; i < Number(nextOrderId); i++) {
